...
- Bei Code-Bezug zuerst Graphify (`graphify query/path/explain`), falls laut `tech-stack.md` verfügbar, dann gezielt lesen
...
### Projektspezifisch (aus `.claude/project-context/tech-stack.md`)
- [ ] Datenabruf über das dort festgelegte State-Management — kein `useState`+`useEffect` für Server-State, sofern verboten
- [ ] Mutations invalidieren über den dort benannten zentralen Mechanismus
- [ ] Keine eigenen Card-/Modal-/Listenzeilen-Strukturen — nur die dort benannten gemeinsamen Komponenten
- [ ] Keine Inline-Labels — nur die dort benannte zentrale Label-/Übersetzungsdatei
- [ ] Neue/geänderte Routen haben eine Berechtigungsentscheidung (Auth/Rollen-Abschnitt in `tech-stack.md`)

Bei Befund: sofort korrigieren, außer der Auftrag erlaubt ausdrücklich das Offenlassen.

Quelle (Ebene 1): Skill Library `dev-testing/core/code-discipline.md`.
